...
i_type = {
    ...
    "bgezal":(0b000001,0b10001,["rs"]),
    "blez":  (0b000110,0b00000,["rs"]),
    "bltz":  (0b000001,0b00000,["rs"]),
    "bltzal":(0b000001,0b10000,["rs"]),
    "bne":   (0b000101,["rs", "rt"]),
    "lb":    (0b100000,["rt", "rs"]),
    "lbu":   (0b100100,["rt", "rs"]),
    "lh":    (0b100001,["rt", "rs"]),
    ...
    "lui":   (0b001111,["rt"]),
    "lw":    (0b100011,["rt", "rs"]),
    "lwl":   (0b100010,["rt", "rs"]), #was rs, rt but this works
    "lwr":   (0b100110,["rt", "rs"]),
    "lwc1":  (0b110001,["rt", "rs"]),
    "ori":   (0b001101,["rt", "rs"]),
    "sb":    (0b101000,["rt", "rs"]),
...
def MakeInstruction(position, **kwargs):
    if 'name' in kwargs and \
        ...
        return PseudoInstruction(position, **kwargs)
    else:
    ...
    def ToBinary(self):
        ...
        if self.name in i_type.keys():
            ...
            if self.label is not None:
                ...
                # horribly hacky. are we a branch?
                if "b" == self.name[0]:
                    b |= (self.imm & 0xFFFF) # imm [was imm>>2]
                else:
        ...
        if self.name in j_type.keys():
            ...
            if self.label is not None:
                ...
                b |= (self.imm & 0x03FFFFFF) # address  was right shift 2
            return b
