...
if 'verbose' in args or 'output' not in args:
    ...
    for j in range(len(binary)/4):
        hexdata = "%02x %02x %02x %02x"%tuple(binary[j*4:j*4+4])
        hexdata_list = hexdata.split(" ")

        for data in hexdata_list:
            result = ""
            for i in range(2):
                if data[i].isalpha():
                    if data[i] == 'a':
                        result += format(10, 'b').zfill(4)
                    elif data[i] == 'b':
                        result += format(11, 'b').zfill(4)
                    elif data[i] == 'c':
                        result += format(12, 'b').zfill(4)
                    elif data[i] == 'd':
                        result += format(13, 'b').zfill(4)
                    elif data[i] == 'e':
                        result += format(14, 'b').zfill(4)
                    elif data[i] == 'f':
                        result += format(15, 'b').zfill(4)
                else:
                    result += format(int(data[i]), 'b').zfill(4)
            print(result)
